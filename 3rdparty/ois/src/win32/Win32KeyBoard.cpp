...
*/
/*
This source was modified by Cory Parks to add additional key modifier support.
*/
...
void Win32Keyboard::_readBuffered()
{
	...
	bool passModifierKeyPress = false;
	bool passModifierKeyRelease = true;
	...
	for(unsigned int i = 0; i < entries; ++i )
	{
		...
		{			
			...
			if( mListener && (passModifierKeyPress || !isModifierKey) )
		...
		else
		{
			...
			//Fire off event
			if( mListener && (passModifierKeyRelease || !isModifierKey) )
...
void Win32Keyboard::_read()
{
	...
	if( isKeyDown(KC_LCONTROL) )
		mModifiers |= LCtrl;
	else if( isKeyDown(KC_RCONTROL) )
		mModifiers |= RCtrl;
	else if( isKeyDown(KC_LSHIFT) )
		mModifiers |= LShift;
	else if( isKeyDown(KC_RSHIFT) )
		mModifiers |= RShift;
	else if( isKeyDown(KC_LMENU) )
		mModifiers |= LAlt;
	else if( isKeyDown(KC_RMENU) )
		mModifiers |= RAlt;
	else if( isKeyDown(KC_LWIN) )
		mModifiers |= LWin;
	else if( isKeyDown(KC_RWIN) )
		mModifiers |= RWin;
}
